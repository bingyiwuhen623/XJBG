namespace Bit
{
    ...
    public class BitSet : ICloneable
    {
        ...
        public static BitSet StrToBitSet(string str)
        {
            ...
            {
                //Debuger.Log("StrToBitSet:str(" + str + ")");
                try
                {
                    byte[] bytes = System.Convert.FromBase64String(str);
                    MemoryStream memoryStream = new MemoryStream(bytes);
                    BinaryReader b = new BinaryReader(memoryStream);
                    Int64 length = b.ReadInt64();
                    uint len1 = (uint)length >> 6;
                    if (((int)length & 0x3f) != 0)
                        len1++;

                    long[] bits = new long[len1];
                    for (int i = 0; i < len1; i++)
                    {
                        bits[i] = b.ReadInt64();
                    }

                    Bit.BitSet bitSet = new Bit.BitSet((int)length);
                    bitSet.bits = bits;
                    return bitSet;
                }
                catch
                {
                    Debuger.LogError("StrToBitSet:str(" + str + ")");
                }
            }
            return new BitSet();
        }
